...
void VSYNC_ISR_Stop(void)
{
    ...
    VSYNC_ISR_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef VSYNC_ISR_INTERRUPT_INTERRUPT_CALLBACK
        VSYNC_ISR_Interrupt_InterruptCallback();
    #endif /* VSYNC_ISR_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void VSYNC_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)VSYNC_ISR__INTC_NUMBER] = address;
}
...
cyisraddress VSYNC_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)VSYNC_ISR__INTC_NUMBER];
}
...
*
*   Note calling VSYNC_ISR_Start or VSYNC_ISR_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after VSYNC_ISR_Start or VSYNC_ISR_StartEx has been called. 
...
uint8 VSYNC_ISR_GetState(void)
{
    ...
    return ((*VSYNC_ISR_INTC_SET_EN & (uint32)VSYNC_ISR__INTC_MASK) != 0u) ? 1u:0u;
}
